...
/**
 * @OA\Tag(
 *     name="Admin - Guide Tags",
 *     description="API Endpoints pour l'administration des tags de guides"
 * )
 */
class GuideTagAdminController extends Controller
{
    /**
     * @OA\Get(
     ...
     *     @OA\Parameter(name="search", in="query", required=false, @OA\Schema(type="string", example="entretien")),
    ...
    public function index(Request $request)
    {
        ...
        $query = GuideTag::withCount('guides');
    ...
    public function stats()
    {
        ...
        $totalTags = GuideTag::count();
        $tagsWithGuides = GuideTag::has('guides')->count();

        return response()->json([
            'total_tags' => $totalTags,
            'tags_with_guides' => $tagsWithGuides,
            'unused_tags' => $totalTags - $tagsWithGuides,
            'top_tags' => GuideTag::withCount('guides')
     ...
     *         @OA\JsonContent(
     *             required={"name"},
     *             @OA\Property(property="name", type="string", example="Réparation")
     *         )
     *     ),
    ...
    public function store(Request $request)
    {
        ...
        $validator = Validator::make($request->all(), [
            'name' => 'required|string|max:100|unique:guide_tags,name',
        ]);
        ...
        while (GuideTag::where('slug', $slug)->exists()) {
            $slug = $originalSlug . '-' . $count;
            $count++;
        }

        $tag = GuideTag::create(['name' => $request->name, 'slug' => $slug]);
    ...
    public function update(Request $request, $id)
    {
        ...
        $validator = Validator::make($request->all(), [
            'name' => 'required|string|max:100|unique:guide_tags,name,' . $id,
        ]);
        ...
        while (GuideTag::where('slug', $slug)->where('id', '!=', $id)->exists()) {
            $slug = $originalSlug . '-' . $count;
            $count++;
        }
    ...
    public function bulkDelete(Request $request)
    {
        ...
        $validator = Validator::make($request->all(), [
            ...
            'ids.*' => 'integer|exists:guide_tags,id',
        ]);
    ...
    public function cleanupUnused()
    {
        ...
        $count = GuideTag::doesntHave('guides')->count();
        GuideTag::doesntHave('guides')->delete();

...
class NotificationService
{
    ...
    protected function sendPushNotification(User $user, Notification $notification, array $options = []): array
    {
        ...
        // (and currently sends a non-stable device_id), so a phone piles up
        // dozens of active tokens — dedupe to the most recently used one per
        // (device_name + device_type), keeping the newest.
    ...
    public function notifyEventCreated(User $user, $event): array
    {
        return $this->sendToUser($user, 'event_created', [
            ...
            'event_date' => optional($event->event_date)->toIso8601String() ?? now()->toIso8601String(),
    ...
    public function notifyEventStartingSoon(User $user, $event): array
    {
        return $this->sendToUser($user, 'event_starting_soon', [
            'event_id' => $event->id,
            'event_title' => $event->title,
            'starts_at' => optional($event->event_date)->toIso8601String() ?? now()->toIso8601String(),
        ], [
            'entity' => $event,
            'priority' => 'high',
        ]);
